...
export class Logger {
    ...
    /** @gpt */
    static response(kind: string, name: string, prompt: string, description:string) {
        process.stdout.write(
        ...
        Logger.token((prompt.length + description.length)/4);
    ...
    /** @gpt */
    static token(total_tokens: number, title:string = 'Total') {
        process.stdout.write(
